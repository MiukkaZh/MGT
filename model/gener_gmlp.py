# This code is modified from https://github.com/lucidrains/g-mlp-pytorch
...
class gMLPGener(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.layers = nn.ModuleList([Residual(PreNorm(dim, gMLPBlock(dim = dim, heads = heads, dim_ff = dim_ff, seq_len = seq_len, attn_dim = attn_dim))) for i in range(depth)])
    ...
    def forward(self, x, mode='mte'):
        ...
        out = out.mean(dim=2)
        out = self.relu(self.avg_bn(out))
        spk_embedding = self.fc(out)
