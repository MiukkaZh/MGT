...
class SimpleBlock(nn.Module):
  ...
  def __init__(self, indim, outdim, half_res, leaky=False):
    ...
    if self.maml:
      ...
      self.BN2 = BatchNorm2d_fw(outdim) # feature-wise transformation at the end of each residual block
    else:
...
class ResNet(nn.Module):
  ...
  def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten=True, leakyrelu=False):
    ...
    # ori: conv(1, 64, 7, 1, 3); bn(64)
    if self.maml:
      conv1 = Conv2d_fw(1, 32, kernel_size=7, stride=2, padding=3, bias=False)
      # conv1 = Conv2d_fw(1, 32, kernel_size=7, stride=1, padding=3, bias=False)
      bn1 = BatchNorm2d_fw(32)
    else:
      conv1 = nn.Conv2d(1, 32, kernel_size=7, stride=2, padding=3, bias=False)
      # conv1 = nn.Conv2d(1, 32, kernel_size=7, stride=1, padding=3, bias=False)
...
def ResNet10(flatten=True, leakyrelu=False):
    return ResNet(SimpleBlock, [1,1,1,1],[32,64,128,256], flatten, leakyrelu)
def ResNet18(flatten=True, leakyrelu=False):
    return ResNet(SimpleBlock, [2,2,2,2],[32,64,128,256], flatten, leakyrelu)
def ResNet34(flatten=True, leakyrelu=False):
    return ResNet(SimpleBlock, [3,4,6,3],[32,64,128,256], flatten, leakyrelu)
